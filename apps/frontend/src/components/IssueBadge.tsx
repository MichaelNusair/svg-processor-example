...
export function IssueBadge({ issue }: { issue: DesignIssue }) {
  const config = ISSUE_CONFIG[issue];
  return <span className={`issue-badge ${config.className}`}>{config.label}</span>;
}

export function IssueList({ issues }: { issues: readonly DesignIssue[] }) {
  if (issues.length === 0) {
  ...
  return (
    <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
      {issues.map((issue) => <IssueBadge key={issue} issue={issue} />)}
    </div>
  );
}
